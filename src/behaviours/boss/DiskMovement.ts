...
import { Disk, DiskState } from '../../models/bosses/Disk';
...
export class DiskMovement extends Behaviour {

  target: Disk;

  private nextState: DiskState;
  ...
  constructor(target: Disk) {
    super('diskMovement', target);
  ...
  onUpdate(deltaTime: number): void {
    switch(this.target.state) {
      case DiskState.ENTER_SCREEN:
        this.enterScreen(deltaTime);
      break;
      case DiskState.ATTACK:
  ...
  private enterScreen(deltaTime: number): void {
    ...
    if(this.target.position.distanceTo(this.targetPosition) <= this.target.speed * deltaTime/1000) {
      ...
      this.target.state = DiskState.MOVE;
    } else {
  ...
  private move(deltaTime: number): void {
    ...
    if(this.target.position.x >= 1400) {
      ...
      this.target.state = DiskState.ATTACK;
    }
