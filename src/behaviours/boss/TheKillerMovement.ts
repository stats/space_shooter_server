...
import { TheKiller, TheKillerState } from '../../models/bosses/TheKiller';
...
export class TheKillerMovement extends Behaviour {

  target: TheKiller;

  private nextState: TheKillerState;
  ...
  constructor(target: TheKiller) {
    super('TheKillerMovement', target);
  ...
  onUpdate(deltaTime: number): void {
    switch(this.target.state) {
      case TheKillerState.ENTER_SCREEN:
        this.enterScreen(deltaTime);
      break;
      case TheKillerState.ATTACK:
        this.attack(deltaTime);
      break;
      case TheKillerState.MOVE:
        this.move(deltaTime);
  ...
  private enterScreen(deltaTime: number): void {
    ...
    if(this.target.position.distanceTo(this.targetPosition) <= this.target.speed * deltaTime/1000) {
      ...
      this.target.state = TheKillerState.MOVE;
    } else {
  ...
  private move(deltaTime: number): void {
    ...
    if(this.target.position.x >= 1400) {
      ...
      this.target.state = TheKillerState.ATTACK;
    }
